...
def toplevel_data(title,button_text,command):
    global idEntry, phoneEntry, nameEntry, emailEntry, genderEntry, courseEntry, statusEntry, screen
    screen = Toplevel()
    screen.title(title)
    screen.grab_set()
    ...
    courseLabel.grid(row=6, column=0, padx=30, pady=15, sticky=W)
    courseEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    courseEntry.grid(row=6, column=1, pady=15, padx=10)

    statusLabel = Label(screen, text='Status', font=('times new roman', 20, 'bold'))
    statusLabel.grid(row=7, column=0, padx=30, pady=15, sticky=W)
    statusEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    statusEntry.grid(row=7, column=1, pady=15, padx=10)
...
def update_data():
    query='update student set name=%s,mobile=%s,email=%s, gender=%s,course=%s,status=%s, date=%s,time=%s where id=%s'
    mycursor.execute(query,(nameEntry.get(),phoneEntry.get(),emailEntry.get(),
                            genderEntry.get(),courseEntry.get(),statusEntry.get(),idEntry.get()))
    con.commit()
...
def export_data():
    ...
    table = pandas.DataFrame(newlist,columns=['Id', 'Name', 'Mobile', 'Email', 'Gender', 'Added Date',
                                      'Added Time','Course','Status'])
...
def search_data():
    query = 'select * from student where id=%s or name=%s or email=%s or mobile=%s  or gender=%s  or course=%s or status=%s'
    mycursor.execute(query, (
    idEntry.get(), nameEntry.get(), emailEntry.get(), phoneEntry.get(), genderEntry.get(),
    courseEntry.get(),statusEntry.get()))
    studentTable.delete(*studentTable.get_children())
    fetched_data=mycursor.fetchall()
    for data in fetched_data:
        studentTable.insert('', END, values=data)
...
def add_data():
    if idEntry.get()=='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or  genderEntry.get()=='' or courseEntry.get()=='' or statusEntry.get()=='':
        ...
        try:
            ...
            mycursor.execute(query,(idEntry.get(),nameEntry.get(),phoneEntry.get(),emailEntry.get(),
                                    genderEntry.get(),date,currenttime,courseEntry.get(),statusEntry.get()))
            con.commit()
...
def connect_database():
    def connect():
        global mycursor, con
        try:
            con = pymysql.connect(host='localhost', user='root', password='anish08032003#')
            mycursor = con.cursor()
        ...
        try:
            ...
            mycursor.execute(query)
            query = 'create table student(id int not null primary key, name varchar(30),mobile varchar(10),email varchar(30),gender varchar(20),date varchar(50), time varchar(50),course varchar(50),status varchar(50)'
            mycursor.execute(query)
        except:
            query = 'use studentmanagementsystem'
            mycursor.execute(query)
...
s='Student Attendance Management System'
sliderLabel=Label(root,font=('arial',26,'italic bold'),width=35)
sliderLabel.place(x=400,y=0)
slider()

connectButton= ttk.Button(root,text='Connect database',command=connect_database)
connectButton.place(x=1350,y=10)

leftFrame=Frame(root)
leftFrame.place(x=100,y=180,width=400,height=600)
...
rightFrame.place(x=350,y=180,width=1150,height=600)


scrollBarX=ttk.Scrollbar(rightFrame,orient=HORIZONTAL)
scrollBarY=ttk.Scrollbar(rightFrame,orient=VERTICAL)

studentTable=ttk.Treeview(rightFrame,columns=('Id','Name','Mobile No','Email','Gender','Added Date','Added Time','Course','Status'),
...
studentTable.column('Status',width=100,anchor=CENTER)
